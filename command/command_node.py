...
class CommandNode():
    ...
    def add_provider(self, provider, uuid):
        ...
        asyncio.run_coroutine_threadsafe(aqmp.initializeQueue(provider.uuid),
                                         aqmp.loop)

    def add_headnode(self, provider):
        self.headNodes.append(provider)

    def select_providers(self, user_location, requiredCPU,
                         requiredMemory):  # user location is a tuple (lat, lon)
        ...
        for ip in self.providers:
            for uuid in self.providers[ip]:
                ...
                best_candidate_providers.append(
                    ProviderCandidate(provider, distance, ramCountAvailable,
                                      cpuCountAvailable))
...
class DaemonHandler(daemon_pb2_grpc.MetricsServicer):
    ...
    def SendStaticMetrics(self, request, context):
        ip = request.clientIP
        if (ip not in self.cm.providers) or (request.uuid
                                             not in self.cm.providers[ip]):
            print("Received static metrics from unknown provider: {}".format(
                request.uuid))
            ...
            print("Received static metrics from known provider: {}".format(
                request.uuid))
    ...
    def SendDynamicMetrics(self, request, context):
        ...
        if ip not in self.cm.providers:
            print("Received dynamic metrics from unknown provider: {}".format(
                request.uuid))
        else:
            print("Received dynamic metrics from known provider: {}".format(
                request.uuid))
            p = self.cm.providers[ip][request.uuid]
...
class JobHandler(user_pb2_grpc.JobServicer):
    ...
    def SendJob(self, request, context):
        ...
        for provider in self.cm.headNodes:
            if provider.static_metrics[
                    "CPUNumCores"] >= HEAD_NODE_CPUS and provider.static_metrics[
                        "MiBRam"] >= HEAD_NODE_RAM:
                headNode = provider
                break
        ...
        print("Sending cluster head join request to provider: {}".format(
            headNode.uuid))
